...
def save_total_compensation_from_xml(xml_url, company_name):
    ...
    total_compensation = root990.find(efile_string('CYSalariesCompEmpBnftPaidAmt'))

    # Write data to text file
    filename = f"total_compensation_{company_name}.txt"  
    f = open(filename, "w")
    f.write(total_compensation.text)