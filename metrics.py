...
def get_total_compensation_from_data_frame(dataFrame):
    ...
    select_value = lambda row: row['TotalCompensationRltdOrgsAmt'] if row['TotalCompensationFilingOrgAmt'] == 0 else row['TotalCompensationFilingOrgAmt']
...
def generate_pay_gap_metric(company):
    ...
    pay_gap = ((average_male_salary - average_female_salary )/ average_male_salary )*100
    percent_male = male_count/(male_count+female_count)
    percent_female = female_count/(male_count+female_count)

    result = {'Average_Female_Salary' : average_female_salary, "Average_Male_Salary": average_male_salary, "Pay_Gap": pay_gap, "percent_male": percent_male, "percent_female": percent_female}
    company_name = csv_file_name[csv_file_name.rindex('_')+1:].removesuffix(".csv")
    print(f'{company_name} Gender Report:' )
    print(f'Percent Male: {percent_male}, Percent Female: {percent_female}, Women are paid {pay_gap:.2f}% less' )
    return result
